...
import {
    ...
    sendAmplitudeTestEvent,
} from '@/lib/amplitude';

export default function AmplitudeScreen() {
    ...
    const handleSendEvent = async () => {
        ...
        try {
            const result = await sendAmplitudeTestEvent(nextClickCount);

            setLastStatus(
                `이벤트를 전송했습니다. 응답 코드: ${result.code}`,
            );
        } catch (error) {
            console.error('Failed to capture Amplitude event', error);
            ...
            <ScrollView
                ...
                <View className="gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4">
                    ...
                    <Text className="text-sm text-amber-800">
                        .env에 EXPO_PUBLIC_AMPLITUDE_API_KEY를 추가한 뒤
                        앱을 다시 빌드하세요.
                    </Text>
                </View>
            </ScrollView>
            ...
            <View className="w-full max-w-[800px] gap-6">
                ...
                <Pressable
                    ...
                    className={`min-h-[52px] items-center justify-center rounded-lg px-6 ${
                        isSending
                            ? 'bg-gray-400'
                            : 'bg-indigo-700 active:opacity-80'
                    }`}>
                ...
                <View className="gap-2 rounded-lg border border-gray-200 bg-gray-50 p-4">
                    ...
                    <Text className="text-sm text-gray-500">
                        event: amplitude_test_button_clicked
                    </Text>
                    <Text className="text-sm text-gray-500">
                        userId: {AMPLITUDE_USER_ID || '설정되지 않음'}
                    </Text>
                    <Text className="text-sm text-gray-500">
                        클릭 횟수: {clickCount}
                    </Text>
